...
export async function getPersonasByShow(data) {
    if(data._links.personas[0].href.substring(35)==="148107") {
      return ({name: data.since, id: 148107, autoDJ:true})
    }
    const persona = await fetchSpinAPI(data._links.personas[0].href)
    return ({name: persona.name, id: persona.id, autoDJ: false})
}
